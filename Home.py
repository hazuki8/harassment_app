...
st.caption("※ PC環境での閲覧を推奨します。スマートフォンでも診断は可能ですが、一部のグラフが見づらくなる可能性があります。")

st.subheader("🎓 本研究の背景と目的")
st.markdown("""
        本システムは、「**ハラスメント判断の曖昧さ**」が引き起こす指導の萎縮や、認識のズレによるトラブルを解消することを目的とした研究用プロトタイプです。\n
        個人の認識を「**法的規範（判例）**」および「**社会的合意（統計）**」と比較することで、自身の判断傾向（厳格さ・寛容さ）を客観的に可視化します。
""")
st.divider()
